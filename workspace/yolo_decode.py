...
def nms(boxes, scores, score_thr=0.25, iou_thr=0.45, max_det=300):
    """Per-class NMS. boxes (N,4) xyxy, scores (N,80).
    ...
    if cand.size == 0:
        return []
    order = cand[np.argsort(-cls_scores[cand])]
    kept_box, kept_cls, kept_score = [], [], []
    for c in order:
        b = boxes[c]
        cl = int(cls_ids[c])
        if kept_box:
            kb = np.asarray(kept_box)
            kc = np.asarray(kept_cls)
            k = kb[kc == cl]
            if k.shape[0]:
                iw = np.minimum(k[:, 2], b[2]) - np.maximum(k[:, 0], b[0])
                ih = np.minimum(k[:, 3], b[3]) - np.maximum(k[:, 1], b[1])
                iw = np.clip(iw, 0.0, None)
                ih = np.clip(ih, 0.0, None)
                inter = iw * ih
                area_b = (b[2] - b[0]) * (b[3] - b[1])
                area_k = (k[:, 2] - k[:, 0]) * (k[:, 3] - k[:, 1])
                iou = inter / (area_k + area_b - inter + 1e-9)
                if (iou > iou_thr).any():
                    continue
        kept_box.append(b)
        kept_cls.append(cl)
        kept_score.append(float(cls_scores[c]))
        if len(kept_box) >= max_det:
            break
    return [{"box": b, "cls": c, "score": s}
            for b, c, s in zip(kept_box, kept_cls, kept_score)]
